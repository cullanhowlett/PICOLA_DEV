...
void initialize_transferfunction(void) {
   ...
   FnlTime = 1 / (1 + Fnl_Redshift);
   DstartFnl = growthD(FnlTime, 1.0);
#endif
...
void initialize_powerspectrum(void) {
  ...
  // For COLA we require a redshift of 0.0 else we require a snapshot at
  // the user defined initial redshift to evolve from.
  if (UseCOLA) {
    InitTime = 1;
  } else {
    //InitTime = 1.0 / (Init_Redshift + 1.0);
    InitTime = 1;
  }
...
void read_power_table(void) {
  ...
  NPowerTable = 0;
  kmin = 2 * M_PI / 0.0001 ;  // 10000 h/Mpc
  kmax = 2 * M_PI / 10000.0;  // 0.0001 h/Mpc
  do {
    if(fscanf(fd, " %lg %lg ", &k, &p) == 2) {
...
void print_spec(void) {
  ...
  if(ThisTask == 0) {
    ...
    gf = growthD(0.001, 1.0) / (1.0 / 0.001);

    DDD = growthD(1.0/(Final_Redshift+1.0), 1.0);

    fprintf(fd, "%12g %12g\n", 0.0, DDD);	// print actual starting redshift and linear growth factor for this cosmology

    kstart = 2 * PI / (1000.0 * (3.085678e24 / UnitLength_in_cm));  // 1000 Mpc/h */
    kend   = 2 * PI / (0.001 * (3.085678e24 / UnitLength_in_cm));   // 0.001 Mpc/h */

    for(k = kstart; k < kend; k *= 1.0025) {
      kf = 0.5;      
      po = PowerSpec(k);
      dl = 4.0 * PI * k * k * k * po;
