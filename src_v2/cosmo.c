...
/* ============================================================*/
/* v1.0: This file contains all the cosmology related routines.*/
/* ============================================================*/

#include "vars.h"
#include "proto.h"

// Converts the velocities to v_{rsd}\equiv (ds/d\eta)/(a H(a))
// ============================================================
void velRSD(double a){
  unsigned int i;
  double fac=(a/Qfactor(a));
  for(i=0;i<NumPart;i++){
    P[i].Vel[0]*=fac;
    P[i].Vel[1]*=fac;
    P[i].Vel[2]*=fac;
...
double growthD(double astart, double aend){ 
    return growthDtemp(aend)/growthDtemp(astart);
}
...
double growthDtemp(double a) {
  ...
  if (fabs(x-1.0) < 1.e-3) {
    hyperP = 0.859596768064608  - 0.1016599912520404*(-1.0 + x)  + 0.025791094277821357*pow(-1.0 + x,2) - 0.008194025861121475*pow(-1.0 + x,3) 
           + 0.0029076305993447644*pow(-1.0 + x,4) - 0.0011025426387159761*pow(-1.0 + x,5) + 0.00043707304964624546*pow(-1.0 + x,6) - 0.0001788889964687831*pow(-1.0 + x,7);
    hyperM = 1.1765206505266006 + 0.15846194123099624*(-1.0 + x) - 0.014200487494738975*pow(-1.0 + x,2) + 0.002801728034399257*pow(-1.0 + x,3) 
           - 0.0007268267888593511*pow(-1.0 + x,4) + 0.00021801569226706922*pow(-1.0 + x,5) - 0.00007163321597397065*pow(-1.0 + x,6) + 0.000025063737576245116*pow(-1.0 + x,7);
  } else {
    if (x < 1.0) {
    ...
    if (x<=1.0/30.0) {
      hyperP = 3.9999999999999996*sqrt(x) - 3.4494794123063865*pow(x,0.6666666666666666) + 0.3999999999999999*pow(x,1.5) - 0.13636363636363635*pow(x,2.5) 
             + 0.07352941176470587*pow(x,3.5) - 0.04755434782608695*pow(x,4.5) + 0.033943965517241374*pow(x,5.5) - 0.02578125*pow(x,6.5) + 0.020436356707317072*pow(x,7.5) 
             - 0.01671324384973404*pow(x,8.5) + 0.013997779702240564*pow(x,9.5) - 0.011945562847590041*pow(x,10.5) + 0.01035003662109375*pow(x,11.5) - 0.009080577904069926*pow(x,12.5);
      hyperM = 0.5714285714285715/sqrt(x) + 2.000000000000001*sqrt(x) - 1.4783483195598794*pow(x,0.66666666666666666) + 0.10000000000000002*pow(x,1.5) - 0.022727272727272735*pow(x,2.5) 
             + 0.009191176470588237*pow(x,3.5) - 0.004755434782608697*pow(x,4.5) + 0.002828663793103449*pow(x,5.5) - 0.0018415178571428578*pow(x,6.5) + 0.0012772722942073172*pow(x,7.5) 
             - 0.0009285135472074472*pow(x,8.5) + 0.0006998889851120285*pow(x,9.5) - 0.0005429801294359111*pow(x,10.5) + 0.0004312515258789064*pow(x,11.5) - 0.00034925299631038194*pow(x,12.5);
    }
  }
    
  if (a>0.2) {
    return sqrt(1.0 + (-1.0 + pow(a,-3))*Omega)*(3.4494794123063873799*pow(-1.0 + 1.0/Omega,0.666666666666666666666666666) 
...
double growthD2v(double a) { 
    ...
    double Om = Omega/(Omega+(1.0-Omega)*a*a*a);
    return Qfactor(a)*(d2/a)*2.0*pow(Om,6./11.);
}
...
double DprimeQ(double a,double nGrowth) {
  // not interested in decay modes in this code.
  double nDecay = 0.0;
  double Nn = 6.0*pow(1.0 - Omega,1.5)/growthDtemp(1.0);
  return pow(decayD(a),-1.0 + nDecay)*pow(growthD(1.0, a),-1.0 + nGrowth)*(nGrowth*Nn- (3.0*(nDecay + nGrowth)*Omega*growthD(1.0, a))/(2.*a));      
}

// The linear growth rate assuming GR
// ====================================
...
// The second-order growth rate assuming GR
// ========================================
