...
// Adapted from the 2LPTic code provided by Marc.
// ===========================================================================================
...
// ============================================================
  ...
  if(!(seedtable = (unsigned int *)malloc(Nmesh * Nmesh * sizeof(unsigned int)))) FatalError(4);
...
// ================================================
  ...
  for(axes = 0; axes < 3; axes++) {
    ...
    /*if(Local_nx > 0) {
      ...
      // send ZA disp
      MPI_Isend(&(disp[axes][0]),sizeof(float_kind)*2*alloc_slice,MPI_BYTE,LeftTask,10,MPI_COMM_WORLD,&request);
      MPI_Recv(&(disp[axes][2*alloc_local]),sizeof(float_kind)*2*alloc_slice,MPI_BYTE,RightTask,10,MPI_COMM_WORLD,&status);
      MPI_Wait(&request,&status);

      // send 2nd order disp
      MPI_Isend(&(disp2[axes][0]),sizeof(float_kind)*2*alloc_slice,MPI_BYTE,LeftTask,10,MPI_COMM_WORLD,&request); 
      MPI_Recv(&(disp2[axes][2*alloc_local]),sizeof(float_kind)*2*alloc_slice,MPI_BYTE,RightTask,10,MPI_COMM_WORLD,&status);      
      MPI_Wait(&request,&status);
    }*/
  ...
  for (n = 0; n < Local_np; n++) {
    for (m = 0; m < Nsample; m++) {
      for (p = 0; p < Nsample; p++) {
        ...
        u = ((n+Local_p_start)*Nmesh)/Nsample;
        v = (m*Nmesh)/Nsample;
        w = (p*Nmesh)/Nsample;
